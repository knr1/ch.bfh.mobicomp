...
/**
 */
public class EstimatedAltitudeProfileView {
	...
	private NumberAxis initXAxis() {
		final NumberAxis xAxis = new NumberAxis(0,
				EstimatedAltitudeProfileView.MAX_DATA_POINTS,
				EstimatedAltitudeProfileView.MAX_DATA_POINTS / 10);
	...
	public EstimatedAltitudeProfileView(final Stage stage) {
		this.init(stage);
		this.prepareTimeline();
	}

	public static void addEstimatedAltitudeData(final double data) {
		EstimatedAltitudeProfileView.dataEstimatedAltitude.add(data);
	}
	...
	private void prepareTimeline() {
		...
		new AnimationTimer() {
			...
			public void handle(final long now) {
				EstimatedAltitudeProfileView.this.addDataToSeries();
			}
		}.start();
	}
	...
	private void addDataToSeries() {
		for (int i = 0; i < 50; i++) { // -- add some new samples to the plot
			if (EstimatedAltitudeProfileView.dataEstimatedAltitude.isEmpty()) {
				break;
			}
			this.estimatedAltitudeSeries.getData().add(
					new LineChart.Data<Number, Number>(
							this.estimatedAltitudeXSeriesDataPosition++,
							EstimatedAltitudeProfileView.dataEstimatedAltitude
									.remove()));
		}
		// remove points to keep us at no more than MAX_DATA_POINTS
		if (this.estimatedAltitudeSeries.getData().size() > EstimatedAltitudeProfileView.MAX_DATA_POINTS) {
